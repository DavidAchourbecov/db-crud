...
class UserService {
    ...
    public async updateTokenInDb(user: Users): Promise<Users> {
        try {
            if (isEmpty(user)) throw new HttpException(400, 'userData is empty');
            const findUser: Users = await this.users.findUnique({ where: { id: user.id } });
            if (!findUser) throw new HttpException(409, "User doesn't exist");
    ...
    public updateToken(user: SignupUser): string {
        try {
            const token = jwt.sign(
                {
                    email: user.email,
    ...
    public async login(userData:LoginUser ): Promise<string> {
              ...
              if (isEmpty(userData)) throw new HttpException(400, 'userData is empty');
              const findUser: Users = await this.users.findUnique({ where: { email: userData.email } });
              if (!findUser) throw new HttpException(409, "Email or password is wrong");
              const isPasswordMatching: boolean = await compare(userData.password, findUser.password);
              if (!isPasswordMatching) throw new HttpException(409, "Email or password is wrong");
